...
import { createS3Client, getPresignedDownloadUrl, uploadObject } from '../../core/storage/storage.service';
  ...
  app.post('/upload', async (req, reply) => {
    ...
    if (!ALLOWED_CONTENT_TYPES.has(contentType)) {
      return reply.code(415).send(
        apiError('UNSUPPORTED_MEDIA_TYPE', `Content-Type '${contentType}' nicht erlaubt.`),
      );
    }
    ...
    if (!rawBody || rawBody.length === 0) {
      return reply.code(422).send(
        apiError('VALIDATION_ERROR', 'Request-Body ist leer.'),
      );
    }
    ...
    try {
      ...
      return reply.code(502).send(
        apiError('STORAGE_ERROR', 'Datei konnte nicht gespeichert werden.'),
      );
    }
  ...
  app.get<{ Params: { id: string } }>('/:id', async (req, reply) => {
    ...
    if (!document) {
      return reply.code(404).send(
        apiError('NOT_FOUND', `Dokument ${req.params.id} nicht gefunden.`),
      );
    }
