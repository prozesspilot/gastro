...
import { buildHyperlinkFormula, buildRow } from '../services/row-builder';
...
describe('M07 row-builder — buildRow()', () => {
  ...
  it('Spalte M baut korrekte =HYPERLINK("url","label")-Formel', () => {
    ...
    expect(cell).toMatch(
      /^=HYPERLINK\("https:\/\/drive\.google\.com\/.+","2026-04-28_RE-2026-1042\.pdf"\)$/,
    );
  });
    ...
    const r = baseReceipt({
      extraction: {
        fields: {
          tax_lines: [
            { rate: 0.07, amount: 1.4 },
            { rate: 0.19, amount: 19.0 }, // dominant
          ],
        },
      },
    });
