...
import { clearHookRunnerDeps, setHookRunnerDeps } from '../../src/core/hooks/hook-runner';
...
SUITE('MVP Pipeline E2E', () => {
  it.skipIf(!worldASchemaPresent)(
    'Pipeline-Stages: extracted → categorized (Override) → completed',
    async () => {
      ...
      try {
        ...
        expect((final?.categorization as { skr_account?: string } | undefined)?.skr_account).toBe(
          '3100',
        );
        // Audit-Events: categorized + completed
        const events =
          (final?.audit as { events?: Array<{ type: string }> } | undefined)?.events ?? [];
  ...
  it.skipIf(!worldASchemaPresent)(
    'OCR_FAILED → status requires_review (via /transition + error_log)',
    async () => {
      const customer = await createTestCustomer(pool);
      try {
        const receipt = await seedReceipt(pool, {
  ...
  it.skipIf(!worldASchemaPresent)(
    'Hook wird nach after_categorization gefeuert (execution geloggt)',
    async () => {
      const customer = await createTestCustomer(pool, {
        custom: {
          supplier_overrides: {
      ...
      try {
        ...
        const execs = execRes.json().data as Array<{
          status: string;
