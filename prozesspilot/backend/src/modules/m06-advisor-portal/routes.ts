...
export async function m06AdvisorPortalRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get<{ Params: { customerId: string } }>('/exports/:customerId', async (req, reply) => {
    ...
    const conditions: string[] = ['r.tenant_id = $1', 'r.customer_id = $2'];
    ...
    const exports = rows.map((r) => ({
      ...
      download_url: r.storage_key ? `/api/v1/receipts/${r.id}/download` : null,
  ...
  app.addHook('onSend', async (_req, reply, payload) => {
    ...
    if (
      deprecatedRoutes.some((r) => url.endsWith(r)) ||
      (url.includes('/receipts/') && url.endsWith('/comment'))
    ) {
