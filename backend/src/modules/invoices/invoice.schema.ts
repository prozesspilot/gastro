...
export function rowToInvoiceResponse(row: InvoiceRow): InvoiceResponse {
  return {
    ...
    amount_netto: Number.parseFloat(row.amount_netto),
    ust_rate: Number.parseFloat(row.ust_rate),
    ust_amount: Number.parseFloat(row.ust_amount),
    amount_brutto: Number.parseFloat(row.amount_brutto),
    pdf_path: row.pdf_path,
    status: row.status as InvoiceStatus,
    paid_at: row.paid_at?.toISOString() ?? null,
    paid_amount: row.paid_amount != null ? Number.parseFloat(row.paid_amount) : null,
