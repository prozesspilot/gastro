...
import { type JobStatus, findJobById, listJobs } from './routing.repository';
import { retryJob } from './routing.service';

// ── Zod-Schemas ───────────────────────────────────────────────────────────────

const jobStatusValues: [JobStatus, ...JobStatus[]] = [
  'queued',
  'running',
  'done',
  'failed',
...
export async function routingRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get('/jobs', async (req, reply) => {
    ...
    if (!tenantId || typeof tenantId !== 'string') {
      return reply.code(400).send(apiError('MISSING_TENANT', 'x-pp-tenant-id fehlt'));
    }

    const parsed = listJobsQuerySchema.safeParse(req.query);
    if (!parsed.success) {
      return reply.code(422).send(apiError('VALIDATION_ERROR', parsed.error.message));
    }
  ...
  app.get('/jobs/:id', async (req, reply) => {
    ...
    if (!job) {
      return reply.code(404).send(apiError('NOT_FOUND', 'Job nicht gefunden'));
    }
  ...
  app.post('/jobs/:id/retry', async (req, reply) => {
    ...
    if (!job) {
      return reply
        .code(409)
        .send(apiError('CONFLICT', 'Job nicht gefunden oder nicht retry-fähig (nur failed|dead)'));
    }
