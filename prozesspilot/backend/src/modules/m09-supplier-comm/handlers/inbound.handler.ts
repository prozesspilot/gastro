...
export function buildInboundHandler() {
  return async function inboundHandler(req: FastifyRequest, reply: FastifyReply): Promise<void> {
    ...
    try {
      ...
      logger.info(
        { from: fromAddr, to: toAddr, subject, referenceId },
        'M09: Inbound-Mail empfangen',
      );
