...
import { appendInputSchema, type CustomerProfile } from '../schemas/append.input';
import { COLUMNS } from '../services/columns';
import { buildRow, buildHeaders, type ExtraColumnDef } from '../services/row-builder';
import { renderTabName, DEFAULT_TAB_TEMPLATE } from '../services/tab-name-resolver';
  ...
  return async function appendHandler(
    ...
    if (!ACCEPTED_INPUT_STATUSES.has(receipt.status)) {
      return reply.code(409).send(
        apiError('CONFLICT', `Receipt-Status '${receipt.status}' nicht akzeptiert für /exports/spreadsheet.`, {
    ...
    try {
      ...
      receipt = await hookRunner.run(
        ...
        { receipt, profile: customer_profile as unknown as CustomerProfile & Record<string, unknown> },
      );
      ...
      const fullColumns = [
        ...COLUMNS,
        ...buildHeaders(extraColumns).map((header) => ({ header })),
      ];
      ...
      const existingRow = await adapter.findRowByReceiptId(ctx, customerId, sheetId, tabName, receipt_id);

      const result = existingRow
        ? await adapter.updateRow(ctx, customerId, sheetId, tabName, receipt_id, existingRow.row_index, row)
      ...
      if (err instanceof HeaderConflictError) {
        ...
        void emitExportEvent(redis, 'pp.receipt.export_failed', {
          receipt_id, customer_id: customerId, status: 'error',
          target: provider, error: err.message, trace_id,
        });
        return reply.code(422).send(
          apiError('HEADER_CONFLICT', err.message, {
            sheet_id: err.sheetId, tab: err.tab, expected: err.expected, actual: err.actual,
          }),
        );
      }
      ...
      void emitExportEvent(redis, 'pp.receipt.export_failed', {
        receipt_id, customer_id: customerId, status: 'error',
        target: provider, error: (err as Error).message, trace_id,
      });
