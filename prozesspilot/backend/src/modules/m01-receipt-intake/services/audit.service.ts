/**
 * M01 — Audit-Service-Wrapper.
 *
 * Schreibt einen Eintrag in audit_log (best-effort, Fehler kippen den
 * Hauptpfad nicht). Wird vom Handler nach erfolgreicher Extraktion und
 * bei requires_review gerufen.
 *
...
export async function writeAudit(db: Pool, entry: AuditEntry): Promise<void> {
  const actor = entry.actor ?? { type: 'system', id: 'M01' };
