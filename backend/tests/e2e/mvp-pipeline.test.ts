...
SUITE('E2E MVP Pipeline (Welt A, direkt ohne n8n)', () => {
  it.skipIf(!worldASchemaPresent)(
    'E2E-01: Happy Path — categorize via Override → complete',
    async () => {
      ...
      try {
        ...
        const after = await pool.query('SELECT status FROM receipts WHERE receipt_id = $1', [
          receipt.receipt_id,
        ]);
  ...
  it.skipIf(!worldASchemaPresent)('E2E-02: Niedrige Konfidenz → requires_review', async () => {
    ...
    try {
      ...
      const r = await pool.query('SELECT status FROM receipts WHERE receipt_id=$1', [
        receipt.receipt_id,
      ]);
  ...
  it.skipIf(!worldASchemaPresent)(
    'E2E-03: Duplikat-Erkennung — UNIQUE(customer_id, sha256)',
    async () => {
      const customer = await createTestCustomer(pool);
      try {
  ...
  it.skipIf(!worldASchemaPresent)(
    'E2E-04: M05 Lexoffice-Idempotenz — receipt mit exports[] wird nicht doppelt gepusht',
    async () => {
      const customer = await createTestCustomer(pool);
      try {
        const receipt = await seedReceipt(pool, {
        ...
        const r = await pool.query('SELECT payload FROM receipts WHERE receipt_id=$1', [
          receipt.receipt_id,
        ]);
