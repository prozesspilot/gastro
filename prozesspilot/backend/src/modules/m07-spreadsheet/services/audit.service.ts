/**
 * M07 — Audit-Service-Wrapper.
 *
 * Schreibt einen Eintrag in audit_log (best-effort, Fehler kippen den
 * Hauptpfad nicht). Spiegelt das Pattern aus M01.
 *
 * Sobald der zentrale auditService aus Phase 2 verfügbar ist, wird dieser
 * Wrapper durch dessen API ersetzt.
 */
...
export async function writeAudit(db: Pool, entry: AuditEntry): Promise<void> {
  const actor = entry.actor ?? { type: 'system', id: 'M07' };
  ...
  try {
    ...
    logger.warn({ err, entry }, 'M07 Audit-Insert fehlgeschlagen');
  }
}
