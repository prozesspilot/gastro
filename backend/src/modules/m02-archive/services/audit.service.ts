 ...
 * M02 — Audit-Wrapper.
 *
 * Schreibt einen Eintrag in `audit_log` (best-effort, Fehler kippen den
 * Hauptpfad nicht). Wird vom Handler nach erfolgreicher Archivierung und
 * im Fehlerfall gerufen.
 *
 * Sobald der zentrale auditService aus Phase 2 verfügbar ist, sollte
 * dieser Wrapper durch dessen API ersetzt werden.
 */
...
export async function writeAudit(db: Pool, entry: AuditEntry): Promise<void> {
  const actor = entry.actor ?? { type: 'system', id: 'M02' };
  ...
  try {
    ...
    logger.warn({ err, entry }, 'M02 Audit-Insert fehlgeschlagen');
  }
}
